...
# endif
  ...
  if (!uid)
  {
    fprintf (stderr,
             "========================================================\n");
    fprintf (stderr, "===== ERROR =====\n");
    fprintf (stderr, "Do NOT run ircd as root.\n");
    fprintf (stderr, "Create a normal user account and run it as that\n");
    fprintf (stderr,
             "========================================================\n");
    exit (-1);
  }
  ...
  booted = TRUE;
  check_dynconf (0);
  (void) fprintf (stderr, "Dynconf initialized\n");
...
/*      if (portnum < 0)
  ...
  */
  ...
  if (!(ssl_capable = initssl ()))
  {
    ...
    fprintf (stderr, "(did you generate your certificate ?)\n");
    fprintf (stderr,
             "Server running with Client SSL disabled, consult the above error log for details.\n");
  }
  else
  {
  ...
  if ((fd = openconf (configfile)) == -1)
  {
    ...
    fprintf (stderr,
             "IRCd Fatal Error: Unable to open %s file!!!\n", configfile);
    fprintf (stderr,
             "========================================================\n");
