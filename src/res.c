 ...
 *
 *  Copyright (C) 1992-2007 by the past and present ircd coders, and others.
 ...
 *  USA
 *
 *  $Id: res.c 985 2007-02-04 20:51:36Z shadowmaster $
 *
 */
...
#if !defined(USE_ADNS) && !defined(_WIN32)      /* See res_win32.c for windows resolver */
...
extern int
res_mkquery (int, char *, int, int, char *, int, struct rrec *, char *, int);
...
init_resolver (int op)
{
  ...
  if (op & RES_INITSOCK)
  {
    ...
    (void) setsockopt (ret, SOL_SOCKET, SO_BROADCAST, (char *) &on,
                       sizeof (on));
  }
#ifdef DEBUG
  if (op & RES_INITDEBG);
...
#ifdef	DEBUG
  Debug ((DEBUG_INFO, "rem_request:Remove %#x at %#x %#x", old, *rptr,
          r2ptr));
#endif
  r2ptr = old;
...
query_name (char *name, int class, int type, ResRQ * rptr)
{
  ...
  r =
    res_mkquery (QUERY, name, class, type, NULL, 0, NULL, buf, sizeof (buf));
  if (r <= 0)
  {
    h_errno = NO_RECOVERY;
    return r;
  ...
  do
  {
    hptr->id = htons (ntohs (hptr->id) + k + (u_short) (tv.tv_usec & 0xffff));
#endif /*
        * LRAND48 
        */
    k++;
       ...
       case T_A:
...
#ifdef DNS_ANS_DEBUG
           sendto_realops_lev (DEBUG_LEV,
                               "DNS_A answer from an acceptable (%s)", acc);
#endif
         }
         hp->h_length = dlen;
         if (ans == 1)
         ...
         if (adr < IRC_MAXADDRS)
         {
           ...
           else
           {
             ...
             memcpy (((char *) hp->h_addr_list[adr].S_ADDR) + 12, dr.S_ADDR,
                     4);
           }
#endif
...
#ifdef DNS_ANS_DEBUG
         if (acc)
           sendto_realops_lev (DEBUG_LEV, "DNS_PTR from an acceptable (%s)",
                               acc);
#endif
...
get_res (char *lp)
{
  ...
  int rc, a, max;
  u3_socklen_t len = sizeof (csin);


  rc =
    recvfrom (resfd, buf, sizeof (buf), 0, (struct sockaddr *) &csin,
              (u3_socklen_t *) & len);
  if (rc <= sizeof (HEADER))
...
#endif
  {
    ...
    else
    {
      ...
      if (!found_match_ip)
      {
        ...
        {
#endif
          if (rptr->cinfo.flags == ASYNC_CLIENT && rptr->cinfo.value.client_p)
          {
  ...
  for (; cp; cp = cp->hnum_next)
  {
    for (i = 0; cp->he.h_addr_list[i]; i++)
    {
      ...
      if (!memcmp (((struct IN_ADDR *) cp->he.h_addr_list[i])->S_ADDR,
                   ((struct IN_ADDR *) ip)->S_ADDR, sizeof (struct IN_ADDR)));
#endif
      {
        cainfo.ca_nu_hits++;
        update_list (NULL, cp);
