 ...
 *  USA
 *
 *  $Id: s_hidehost.c 985 2007-02-04 20:51:36Z shadowmaster $
 *
 */
...
make_virthost (char *curr, char *host, char *new)
{
  ...
  hash[0] =
    ((crc32 ((const unsigned char *) parv[3], strlen (parv[3])) +
      KEY2) ^ KEY) ^ KEY3;
  hash[1] =
    ((crc32 ((const unsigned char *) parv[2], strlen (parv[2])) +
      KEY2) ^ KEY) ^ KEY3;
  ...
  if (parc2 == 4)
  {
    ...
    {
      hash[2] =
        ((crc32 ((const unsigned char *) parv[1], strlen (parv[1])) +
          KEY2) ^ KEY) ^ KEY3;
      hash[3] =
        ((crc32 ((const unsigned char *) parv[0], strlen (parv[0])) +
          KEY2) ^ KEY) ^ KEY3;
      ...
      ircsprintf (mask, "%x.%x.%x.%xX", hash[0], hash[1], hash[2], hash[3]);
    }
    else
    {
...
void
make_opervirthost_acc (char *new, int level)
{
  char *host = NULL;
  ...
  {
    host = SERVCOADMIN_HOST;
  }
  else if (level == 3)
  {
    host = GUESTADMIN_HOST;
  }
  else if (level == 2)
  {
    host = OPER_HOST;
  ...
  {
    /*
     * Danger will robinson. This should never happen.
     */
    return;
  }
...
#ifdef INET6
/*
 * ipv6 hidehost added by Willow
 *
 + Using alot of make_virthost only with ipv6 stuff
 */
void
make_ipv6virthost (char *curr, char *host, char *new)
{
  ...
  /* crc32 and hashing blahblahblaaah */
  hash[0] = ((crc32 (parv[0], strlen (parv[0])) + KEY2) ^ KEY) ^ KEY3;
  hash[1] = ((crc32 (parv[1], strlen (parv[1])) + KEY2) ^ KEY) ^ KEY3;

  /* Bitshifting as always */
  hash[0] <<= 2;
  hash[0] >>= 2;
  ...
  {
    hash[2] = ((crc32 (parv[2], strlen (parv[2])) + KEY2) ^ KEY) ^ KEY3;
    hash[3] = ((crc32 (parv[3], strlen (parv[3])) + KEY2) ^ KEY) ^ KEY3;
    hash[4] = ((crc32 (parv[4], strlen (parv[4])) + KEY2) ^ KEY) ^ KEY3;
    hash[5] = ((crc32 (parv[5], strlen (parv[5])) + KEY2) ^ KEY) ^ KEY3;
    hash[6] = ((crc32 (parv[6], strlen (parv[6])) + KEY2) ^ KEY) ^ KEY3;
    hash[7] = ((crc32 (parv[7], strlen (parv[7])) + KEY2) ^ KEY) ^ KEY3;
    ...
    /* We dont reverse the mask since clients should parse this correct */
    ircsprintf (mask, "%x:%x:%x:%x:%x:%x:%x:%x",
...
/* We need to expand the ipv6 properly in order to get the entire mask - Willow */
void
ip62arr (char *string, char *dest)
{
  ...
  for (i = 0; i < length + 1; i++)
  {
    if (string[i] == ':')
    {
      ...
      if (string[i + 1] == ':')
      {
        /* we've encountered the whacky :: , fix it */
        if (string[i + 2] == '\0')
        {
