...
m_kline (aClient * client_p, aClient * source_p, int parc, char *parv[])
{
  ...
  if ((out = open(filename, O_RDWR | O_APPEND | O_CREAT,S_IRUSR|S_IWUSR)) == -1)
  {
...
# endif
  ...
  if ((in = open (filename, O_RDONLY,S_IRUSR|S_IWUSR)) == -1)
  {
...
do_pending_klines ()
{
  ...
  /* Open klinefile */
  if ((fd = open (klinefile, O_WRONLY | O_APPEND,S_IRUSR|S_IWUSR)) == -1)
  {
    sendto_realops
...
#endif
    if ((out = open (configfile, O_RDWR | O_APPEND | O_CREAT,S_IRUSR|S_IWUSR)) == -1)
    {
      sendto_one (source_p, ":%s NOTICE %s :Problem opening %s ",
